...
int rtk_eMMC_init( void )
{
	...
	REG32(SYS_PLL_EMMC1) = 0x83;     //LDO1.8V
...
int rtk_eMMC_write( unsigned int blk_addr, unsigned int data_size, unsigned int * buffer, unsigned int auto_sel_part )
{
	...
	//if (total_blk_cont > 1){
		...
		while (remain_blk_cnt) {
			...
			if (cur_blk_cnt > 1)
				ret_err = emmc_read_write_ip(MMC_WRITE_MULTIPLE_BLOCK, cur_blk_addr, (UINT32)buffer + ((cur_blk_addr - blk_addr) << 9), cur_blk_cnt << 9);
			else
				ret_err = emmc_read_write_ip(MMC_WRITE_BLOCK, cur_blk_addr, (UINT32)buffer + ((cur_blk_addr - blk_addr) << 9), cur_blk_cnt << 9);
...
int rtk_eMMC_read( unsigned int blk_addr, unsigned int data_size, unsigned int * buffer, unsigned int auto_sel_part )
{
	...
	//if (total_blk_cont > 1){
		...
		while (remain_blk_cnt) {
			...
			remain_blk_cnt -= cur_blk_cnt;
			if (cur_blk_cnt > 1)
				ret_err = emmc_read_write_ip(MMC_READ_MULTIPLE_BLOCK, cur_blk_addr, (UINT32)buffer + ((cur_blk_addr - blk_addr) << 9), cur_blk_cnt << 9);
			else
				ret_err = emmc_read_write_ip(MMC_READ_SINGLE_BLOCK, cur_blk_addr, (UINT32)buffer + ((cur_blk_addr - blk_addr) << 9), cur_blk_cnt << 9);
			cur_blk_addr += cur_blk_cnt;
			if (ret_err ) return 0;
