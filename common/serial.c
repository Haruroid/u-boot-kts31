...
static struct serial_device *get_current(void)
{
	...
	if (!(gd->flags & GD_FLG_RELOC) || !serial_current) {
        ...
        if(gd->flags & GD_FLG_MTMODE)
            dev = get_serial_console(MT_ACCESS_UART);
        else
#endif
...
#ifdef CONFIG_MODULE_TEST
static struct serial_device *get_uart(char uart)
{
    struct serial_device *dev;
...
void switch_into_mtmode()
{
    gd->flags = gd->flags | GD_FLG_MTMODE;

    return;
}

void switch_exit_mtmode()
{
    gd->flags = gd->flags & (~GD_FLG_MTMODE);
...
{
	return get_uart(uart)->init();
}

void serial_puts_uart(const char *s, char uart)
{
    get_uart(uart)->puts(s);
}
#endif
