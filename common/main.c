...
int abortboot(int bootdelay)
{
    ...
    if (factory_read(BOOT_RECOVERY_FILE_NAME, &dst_addr, &dst_length)) {
        ...
        if (dst_length == 0) {            
            factory_delete(BOOT_RECOVERY_FILE_NAME);
            factory_save();
...
#define DETECT_KEY_RETRY_COUNT 10
  ...
  unsigned int key=0;
	...
	ft_init_gpio();
        ...
        setISOGPIO(CONFIG_LTE_PWR,0);
        setGPIO(CONFIG_FAN_CTR1,0);
        setGPIO(CONFIG_FAN_CTR2,0);
        setGPIO(CONFIG_SATA_HDD_POWER,0);
...
start = get_timer(0);
	...
	setISOGPIO(CONFIG_WPD_LED_RED,1);
	char* p;
	// check power status!
	p=getenv("PowerStatus");
	while(!(strstr(p,"on"))&& abort==0)
	{
		int adbNUM=0;
		char ret1;
		ret1=getISOGPIO(CONFIG_POWER_GPIO_NUM);
		if(!(ret1))
		{
			setenv("PowerStatus", "on");
			saveenv();
			saveenv();
			p=getenv("PowerStatus");
			if( p == NULL ||(!(strstr(p,"on"))))
			{
				setenv("PowerStatus", "on");
				saveenv();
				saveenv();
	...
	setISOGPIO(CONFIG_POWER_LED_GREEN,0);
	setISOGPIO(CONFIG_POWER_LED_RED,0);
	setISOGPIO(CONFIG_LED_CTR1,1);
	setISOGPIO(CONFIG_LED_CTR2,1);
	setISOGPIO(CONFIG_LTE_PWR,1);	
