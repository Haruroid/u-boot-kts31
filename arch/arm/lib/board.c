...
void board_init_r(gd_t *id, ulong dest_addr)
{
	...
	debug("golden_sec_end:0x%08x\n", (unsigned)golden_sec_end);
	gd->tlb_addr = golden_sec_end;
	gd->tlb_addr &= ~(0x10000 - 1);
	gd->tlb_addr += 0x10000;
	debug("PAGE-TBL moved to 0x%08x\n", (unsigned int)gd->tlb_addr);
#endif
	/* Enable caches */
	enable_caches();
