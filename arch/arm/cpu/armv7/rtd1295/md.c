...
int md_memcpy(void *dst, void *src, unsigned int len)
{
	...
	sync();
	flush_cache(cmd,4* sizeof(unsigned int)*num_of_cmd);

	rtd_outl(MD_KMQ_CNTL,0x6);
	rtd_outl(MD_KMQ_INT_STATUS,0x3e);
	rtd_outl(MD_KMQBASE , (unsigned int)cmd);
	rtd_outl(MD_KMQRDPTR, (unsigned int)cmd);
	rtd_outl(MD_KMQWRPTR,(unsigned int)cmd + num_of_cmd * sizeof(unsigned int) * 4);
	rtd_outl(MD_KMQLIMIT,(rtd_inl(MD_KMQWRPTR) + sizeof(unsigned int) * 4));

	rtd_outl(MD_KMQ_CNTL, 0x7);
	i = 100000;
	while (i--) {
		// check idle
		if (rtd_inl(MD_KMQ_CNTL) & 0x8)
			return 0;
		
		// check error
		if (rtd_inl(MD_KMQ_INT_STATUS) & 0x2) {
			printf("[ERROR] opcode error\n");
			return -1;
		}

		if (rtd_inl(MD_KMQ_INT_STATUS) & 0x4) {
