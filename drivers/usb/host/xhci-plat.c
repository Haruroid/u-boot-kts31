...
static u32 dwc3_readl(struct ctrl_data dwc3, u32 offset)
{
	 ...
	 */
	return __raw_readl((volatile u32*)(dwc3.dwc3_addr + REG_USB3BASE + (offset - DWC3_GLOBALS_REGS_START)));
}
...
static void dwc3_writel(struct ctrl_data dwc3, u32 offset, u32 value)
{
	 ...
	 * However, the offsets are given starting from xHCI address space.
	 */
	__raw_writel(value, (volatile u32*)(dwc3.dwc3_addr + REG_USB3BASE + (offset - DWC3_GLOBALS_REGS_START)));
}
...
static int rtk_usb_phy3_wait_vbusy(struct ctrl_data dwc3)
{
	return xhci_usb_utmi_wait_register(dwc3.wrap_addr + REG_MDIO_CTL, (1<<7), 0);
}

static u32 rtk_xhci_usb3_phy_read(struct ctrl_data dwc3, char addr)
{
...
static int rtk_xhci_usb3_phy_write(struct ctrl_data dwc3, char addr, u16 data)
{
	volatile unsigned int regVal;
...
void usb3_phy_setting(struct ctrl_data dwc3)
{
	int i;
...
#else
	rtk_xhci_usb3_phy_write(dwc3, 0x00, 0x4008);
	rtk_xhci_usb3_phy_write(dwc3, 0x01, 0xA878);
	rtk_xhci_usb3_phy_write(dwc3, 0x02, 0x6046);
	rtk_xhci_usb3_phy_write(dwc3, 0x03, 0x2771);
	rtk_xhci_usb3_phy_write(dwc3, 0x04, 0xB2F5);
	rtk_xhci_usb3_phy_write(dwc3, 0x05, 0x2AD7);
	rtk_xhci_usb3_phy_write(dwc3, 0x06, 0x0001);
	rtk_xhci_usb3_phy_write(dwc3, 0x07, 0x3200);
	if (soc_is_rtk1195() && (realtek_rev() == RTK1195_REV_A)) {
		rtk_xhci_usb3_phy_write(dwc3, 0x08, 0x3590);
		rtk_xhci_usb3_phy_write(dwc3, 0x09, 0x325C);
		rtk_xhci_usb3_phy_write(dwc3, 0x0A, 0xD643);
		rtk_xhci_usb3_phy_write(dwc3, 0x0B, 0xE909);
	}
	else {	// REV_B
		rtk_xhci_usb3_phy_write(dwc3, 0x08, 0x3592);
		rtk_xhci_usb3_phy_write(dwc3, 0x09, 0x325C);
		rtk_xhci_usb3_phy_write(dwc3, 0x0A, 0xD643);
		rtk_xhci_usb3_phy_write(dwc3, 0x0B, 0xA909);
	}
	rtk_xhci_usb3_phy_write(dwc3, 0x0C, 0xC008);
	rtk_xhci_usb3_phy_write(dwc3, 0x0D, 0xFF28);
	rtk_xhci_usb3_phy_write(dwc3, 0x0E, 0x2010);
	rtk_xhci_usb3_phy_write(dwc3, 0x0F, 0x8000);
	rtk_xhci_usb3_phy_write(dwc3, 0x10, 0x000C);
	rtk_xhci_usb3_phy_write(dwc3, 0x11, 0x4C00);
	rtk_xhci_usb3_phy_write(dwc3, 0x12, 0xFC00);
	rtk_xhci_usb3_phy_write(dwc3, 0x13, 0x0C81);
	rtk_xhci_usb3_phy_write(dwc3, 0x14, 0xDE01);
	rtk_xhci_usb3_phy_write(dwc3, 0x15, 0x0000);
	rtk_xhci_usb3_phy_write(dwc3, 0x16, 0x0000);
	rtk_xhci_usb3_phy_write(dwc3, 0x17, 0x0000);
	rtk_xhci_usb3_phy_write(dwc3, 0x18, 0x0000);
	if (soc_is_rtk1195() && (realtek_rev() == RTK1195_REV_A)) {
		rtk_xhci_usb3_phy_write(dwc3, 0x19, 0x7004);
	}
	else {	// REV_B
		rtk_xhci_usb3_phy_write(dwc3, 0x19, 0x3804);
	}
	rtk_xhci_usb3_phy_write(dwc3, 0x1A, 0x1260);
	rtk_xhci_usb3_phy_write(dwc3, 0x1B, 0xFF0C);
	rtk_xhci_usb3_phy_write(dwc3, 0x1C, 0xCB1C);
	rtk_xhci_usb3_phy_write(dwc3, 0x1D, 0xA03F);
	rtk_xhci_usb3_phy_write(dwc3, 0x1E, 0xC200);
	rtk_xhci_usb3_phy_write(dwc3, 0x1F, 0x9000);
	rtk_xhci_usb3_phy_write(dwc3, 0x20, 0xD4FF);
	rtk_xhci_usb3_phy_write(dwc3, 0x21, 0xAAFF);
	rtk_xhci_usb3_phy_write(dwc3, 0x22, 0x0051);
	rtk_xhci_usb3_phy_write(dwc3, 0x23, 0xDB60);
	rtk_xhci_usb3_phy_write(dwc3, 0x24, 0x0800);
	rtk_xhci_usb3_phy_write(dwc3, 0x25, 0x0000);
	rtk_xhci_usb3_phy_write(dwc3, 0x26, 0x0004);
	rtk_xhci_usb3_phy_write(dwc3, 0x27, 0x01D6);
	if (soc_is_rtk1195() && (realtek_rev() == RTK1195_REV_A)) {
		rtk_xhci_usb3_phy_write(dwc3, 0x28, 0xF882);
	}
	else {	// REV_B
	//	rtk_xhci_usb3_phy_write(0x28, 0xF842);
		rtk_xhci_usb3_phy_write(dwc3, 0x28, 0xF840);
	}
	rtk_xhci_usb3_phy_write(dwc3, 0x29, 0x3080);
	rtk_xhci_usb3_phy_write(dwc3, 0x2A, 0x3083);
	rtk_xhci_usb3_phy_write(dwc3, 0x2B, 0x2038);
	rtk_xhci_usb3_phy_write(dwc3, 0x2C, 0xFFFF);
	rtk_xhci_usb3_phy_write(dwc3, 0x2D, 0xFFFF);
	rtk_xhci_usb3_phy_write(dwc3, 0x2E, 0x0000);
	rtk_xhci_usb3_phy_write(dwc3, 0x2F, 0x0040);
	rtk_xhci_usb3_phy_write(dwc3, 0x09, 0x325C);
	rtk_xhci_usb3_phy_write(dwc3, 0x09, 0x305C);
	rtk_xhci_usb3_phy_write(dwc3, 0x09, 0x325C);
#endif
...
{
#if 0
	if (soc_is_rtk1195() && (realtek_rev() < SOC_REV_C)) {
                if ((state & USB_PORT_STAT_CONNECTION) && (connected==0)) {
			connected = 1;
			debug("*********set A03E ******\n");
        		rtk_xhci_usb3_phy_write(dwc3, 0x1D, 0xA03E);
                }	
...
#endif
static char rtk_xhci_usb2_phy_read(struct ctrl_data dwc3, char addr)
{
	volatile unsigned int regVal;
...
static int rtk_xhci_usb2_phy_write(struct ctrl_data dwc3, char addr, char data)
{
	volatile unsigned int regVal;
...
void usb2_phy_setting(struct ctrl_data dwc3)
{
#if 1
	int i;
...
#else
	int i;
	rtk_xhci_usb2_phy_write(dwc3, 0xf4, 0x9b);	// set page 0

	for (i=0; i<ARRAY_SIZE(phy_page0_default_setting); i++) {
		if (rtk_xhci_usb2_phy_write(dwc3, phy_page0_default_setting[i].addr, phy_page0_default_setting[i].data)) {
			printf("[%s:%d], Error : addr = 0x%x, value = 0x%x\n",
				__FUNCTION__, __LINE__,
	...
	rtk_xhci_usb2_phy_write(dwc3, 0xf4, 0xbb);	// set page 1

	for (i=0; i<ARRAY_SIZE(phy_page1_default_setting); i++) {
		if (rtk_xhci_usb2_phy_write(dwc3, phy_page1_default_setting[i].addr, phy_page1_default_setting[i].data)) {
			printf("[%s:%d], Error : addr = 0x%x, value = 0x%x\n",
				__FUNCTION__, __LINE__,
...
int xhci_hcd_init(int index, struct xhci_hccr **hccr, struct xhci_hcor **hcor)
{
	...
	// 3. irq ?
	int ret;
	*hccr = (struct xhci_hccr *)realtek_get_base_usb_xhci(index);
	*hcor = (struct xhci_hcor *)((uint32_t) *hccr
	...
	ctrl.dwc3_addr = realtek_get_base_usb_xhci(index);
	ctrl.wrap_addr = realtek_get_base_usb_xhci_phy(index);
	ctrl.u2phy.size = realtek_get_dwc3_u2phy_size(index)[0];
	ctrl.u2phy.addr = realtek_get_dwc3_u2phy_addr(index);
	ctrl.u2phy.data = realtek_get_dwc3_u2phy_data(index);
	ctrl.u3phy.size = realtek_get_dwc3_u3phy_size(index)[0];
	ctrl.u3phy.addr = realtek_get_dwc3_u3phy_addr(index);
	ctrl.u3phy.data = realtek_get_dwc3_u3phy_data(index);

	debug("%s dwc3 dwc3_addr=%p, wrap_addr =%p\n", __func__, ctrl.dwc3_addr, ctrl.wrap_addr);
	...
	mdelay(100);
	__raw_writel(0x7, (volatile u32*)(ctrl.wrap_addr + 0x58/*0x18013258*/));		//barry???
	mdelay(100);
	__raw_writel(0x606, (volatile u32*)(ctrl.wrap_addr + 0x70/*0x18013270*/));	//barry set Dpm 1'b1
	mdelay(100);
