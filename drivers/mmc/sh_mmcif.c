					...
					struct mmc_cmd *cmd)
{
	if (cmd->flags & MMC_RSP_136) {
		cmd->resp[0] = sh_mmcif_read(&host->regs->ce_resp3);
		cmd->resp[1] = sh_mmcif_read(&host->regs->ce_resp2);
		cmd->resp[2] = sh_mmcif_read(&host->regs->ce_resp1);
		cmd->resp[3] = sh_mmcif_read(&host->regs->ce_resp0);
		debug(" RESP %08x, %08x, %08x, %08x\n", cmd->resp[0],
			 cmd->resp[1], cmd->resp[2], cmd->resp[3]);
	} else {
		cmd->resp[0] = sh_mmcif_read(&host->regs->ce_resp0);
	}
}
...
static void sh_mmcif_get_cmd12response(struct sh_mmcif_host *host,
					struct mmc_cmd *cmd)
{
	cmd->resp[0] = sh_mmcif_read(&host->regs->ce_resp_cmd12);
}
