...
#include <asm/arch-rtd1295/cpu.h>
...
int setRandomMemory(unsigned int dmaddr, unsigned int length)
{
	int i=0;
        for(i=0;i<(length/4);i++)
        {
                *(u32 *)(dmaddr+(i*4)) = (u32)((i+0x78*0x80)*0x13579bdf);
                sync();
        }
...
static void rtkemmc_backup_registers()
{
...
static void set_cmd_info(struct e_device_type *card,struct mmc_cmd * cmd,
...
int rtkemmc_send_status(u32* resp, uint show_msg, int bIgnore)
{
    ...
    {
		bMalloc=1;
		card = (struct e_device_type*)malloc(sizeof( e_device_type));
    }
...
int rtk_eMMC_read( unsigned int blk_addr, unsigned int data_size, unsigned int * buffer )
{
    ...
    UINT8 ret_state=0;
    UINT32 tar_dest=0, bRetry=0, resp = 0;
	...
	while( total_blk_cont ) {
        ...
        if (IS_SRAM_ADDR(buffer))
			...
			if (curr_xfer_blk_cont == 1)
        		ret_err = rtkemmc_Stream(0xa0000340|MMC_READ_SINGLE_BLOCK, address, buffer, curr_xfer_blk_cont << 9,0);
			else
				ret_err = rtkemmc_Stream(0xa0001340|MMC_READ_MULTIPLE_BLOCK, address, buffer, curr_xfer_blk_cont << 9,0);
        } 
		if( ret_err ) {
			return 0;
		}
...
void rtkemmc_set_div( unsigned int set_div )
{
    ...
    isb();
    sync();
    wait_done_timeout(CR_EMMC_CMD, 0x80000000, 0x0);
    sync();
...
int rtkemmc_get_mode_selection()
{
   //tbd
    return;
}
...
void make_ip_des(UINT32 dma_addr, UINT32 dma_length){
	...
	flush_cache(rw_descriptor, 16*MAX_DESCRIPTOR_NUM);
...
int rtkemmc_Stream_Cmd( struct rtk_cmd_info * cmd_info, unsigned int bIgnore )
{
	...
	while (remain_blk_cnt) {
		...
		ret_err = rtkemmc_Stream(cmd_idx|cmd_info->cmd_para, cur_blk_addr, (UINT32)data + ((cur_blk_addr - cmd_info->cmd->arg) << 9), cur_blk_cnt << 9,bIgnore);
		if ((!ret_err)&&(bIgnore)) 
		{
			return 0;
		}
...
void card_stop(){
        ...
        wait_done_timeout(CR_EMMC_CMD, 0x80000000,0);
        cr_writel(gRegTbl.emmc_clk_div, CR_EMMC_CLKDIV);
        sync();
...
int rtkemmc_send_cmd18()
{
	...
	setRandomMemory(crd_tmp_buffer,0x400);
	MMCPRINTF("*** %s %s %d ***\n", __FILE__, __FUNCTION__, __LINE__);
 ...
 *
 *******************************************************/
int rtkemmc_show_cid( e_device_type *card )
{
	int i;
	unsigned int sn;
...
int rtk_micron_eMMC_write( unsigned int blk_addr, unsigned int data_size, unsigned int * buffer )
{
	...
	while( total_blk_cont ) {
		...
		flush_cache((unsigned long)buffer, curr_xfer_blk_cont << 9);
		ret_err = rtkemmc_Stream( address, curr_xfer_blk_cont, MMC_CMD_MICRON_60, buffer, 0 );
...
RETRY_RD_CMD:
    ...
    ret_err = rtkemmc_Stream( address, total_blk_cont, MMC_CMD_MICRON_60, buffer,0 );  //tbd : not verify
...
int rtk_micron_eMMC_read( unsigned int blk_addr, unsigned int data_size, unsigned int * buffer )
{
	...
	while( total_blk_cont ) {
		...
		flush_cache((unsigned long)buffer, curr_xfer_blk_cont << 9);
        	ret_err = rtkemmc_Stream( address, curr_xfer_blk_cont, MMC_CMD_MICRON_61, buffer,0 );
...
int rtk_eMMC_write( unsigned int blk_addr, unsigned int data_size, unsigned int * buffer )
{
	...
	while( total_blk_cont ) {
		...
		if (IS_SRAM_ADDR(buffer))
			...
			if (curr_xfer_blk_cont == 1)
				ret_err = rtkemmc_Stream(0xa0000740|MMC_WRITE_BLOCK, address, buffer, curr_xfer_blk_cont << 9,0);
			else
				ret_err = rtkemmc_Stream(0xa0001740|MMC_WRITE_MULTIPLE_BLOCK, address, buffer, curr_xfer_blk_cont << 9,0);
		} 
...
int rtkemmc_send_wp_type( unsigned int blk_addr, unsigned int * buffer )
{
    ...
    if (IS_SRAM_ADDR(buffer))
    	...
    	ret_err = rtkemmc_Stream(0xa0000340|MMC_SEND_WRITE_PROT_TYPE, address, buffer, 8,0);
