...
int mmc_send_op_cond(struct mmc *mmc)
{
	...
	do {
		...
		udelay(1000);
		flush_cache((unsigned long)cmd.resp[0], 8);
		#ifdef MMC_DEBUG
...
{
	MY_CLR_ALIGN_BUFFER();
	MY_ALLOC_CACHE_ALIGN_BUFFER(char, ext_csd,  CSD_ARRAY_SIZE*2);
	char cardtype;
	int err=0;
	...
	switch(mode)
	{
		case MODE_SD20:
			...
			if (chg_type & CHANGE_FREQ_CARD)
				err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HS_TIMING, EXT_CSD_TIMING_HS, 0);
		...
		case MODE_SD30:
			...
			if (chg_type & CHANGE_FREQ_CARD)
				err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HS_TIMING, EXT_CSD_TIMING_HS200, 0);
			if (chg_type & CHANGE_FREQ_HOST)
...
int mmc_startup(struct mmc *mmc)
{
	...
	MY_CLR_ALIGN_BUFFER();
	MY_ALLOC_CACHE_ALIGN_BUFFER(char, outBlk, CSD_ARRAY_SIZE);
	MY_ALLOC_CACHE_ALIGN_BUFFER(char, rcvBlk, CSD_ARRAY_SIZE);
	MY_ALLOC_CACHE_ALIGN_BUFFER(char, outTwoBlk, CSD_ARRAY_SIZE*2);
	MY_ALLOC_CACHE_ALIGN_BUFFER(char, ext_csd, CSD_ARRAY_SIZE);
	MY_ALLOC_CACHE_ALIGN_BUFFER(char, test_csd, CSD_ARRAY_SIZE);
...
CMD_RETRY:
	...
	cmd.opcode = mmc_host_is_spi(mmc) ? MMC_CMD_SEND_CID :
...
REINIT:
    ...
    flush_cache((unsigned long)cmd.resp, sizeof(unsigned int)*4);
...
//1295 not support ddr50
	...
	printf("[LY] sdr50 : %d\n", err);
		...
		if (err)
			...
			mmc->erase_grp_size =
			      ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE] * 512 * 1024;
			mmc->wpg_size = mmc->erase_grp_size * ext_csd[EXT_CSD_HC_WP_GRP_SIZE];
			printf("[HC] WPG_SIZE = %u\n", mmc->wpg_size);
		}
#endif
...
int mmc_select_ddr50(struct mmc *mmc,char* ext_csd)
{
	...
	if (err)
	{
		printf("host set speed to ddr fail\n");
		return -5;
	}
	sync();
...
int getMMCCapability()
{
        char buf[256], *start;
        char *cmdline = getenv("bootargs");

	//force supporting hs200 by default
	return 1;
}
...
// just decrease the initialized time
...
CMD0_RETRY:
	...
	err = mmc_startup(mmc);
	MMCPRINTF("exit from mmc_startup(),err=%d\n",err);
	if (err)
		mmc->has_init = 0;
	else
		mmc->has_init = 1;
#if defined(CONFIG_RTD299X) || defined(CONFIG_RTD1195) || defined(CONFIG_RTD1295)
	rtkemmc_data_sync(mmc);
