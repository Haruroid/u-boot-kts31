...
static int mmc_request(struct mmc *mmc, struct mmc_cmd *cmd,
	...
	if (mmc_stat & CC_MASK) {
		...
		if (cmd->flags & MMC_RSP_PRESENT) {
			if (cmd->flags & MMC_RSP_136) {
				/* response type 2 */
				cmd->resp[3] = readl(&mmc_base->rsp10);
				cmd->resp[2] = readl(&mmc_base->rsp32);
				cmd->resp[1] = readl(&mmc_base->rsp54);
				cmd->resp[0] = readl(&mmc_base->rsp76);
			} else
				/* response types 1, 1b, 3, 4, 5, 6 */
				cmd->resp[0] = readl(&mmc_base->rsp10);
		}
	}
