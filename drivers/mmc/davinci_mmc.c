 ...
 */
static int
dmmc_request(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data)
{
	struct davinci_mmc *host = mmc->priv;
	...
	if (mmcstatus & MMCST0_RSPDNE) {
		...
		if (cmd->flags & MMC_RSP_136) {
			cmd->resp[0] = get_val(&regs->mmcrsp67);
			cmd->resp[1] = get_val(&regs->mmcrsp45);
			cmd->resp[2] = get_val(&regs->mmcrsp23);
			cmd->resp[3] = get_val(&regs->mmcrsp01);
