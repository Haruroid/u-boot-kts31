		...
		struct mmc_data *data)
{
	...
	int ret = 0;
	debug("%s:cmd%d %x %x\n", __func__,
	      cmd->cmdidx, cmd->cmdarg, cmd->flags);
	spi_claim_bus(spi);
	spi_cs_activate(spi);
	...
	if (r1 == 0xff) { /* no response */
		...
		ret = TIMEOUT;
		goto done;
	} else if (cmd->flags == MMC_RSP_R2) {
		r1 = mmc_spi_readdata(mmc, cmd->resp, 1, 16);
		for (i = 0; i < 4; i++)
			cmd->resp[i] = swab32(cmd->res[i]);
		...
		case MMC_CMD_SEND_OP_COND:
			cmd->resp[0] = (r1 & R1_SPI_IDLE) ? 0 : OCR_BUSY;
			break;
		case SD_CMD_SEND_IF_COND:
		case MMC_CMD_SPI_READ_OCR:
			spi_xfer(spi, 4 * 8, NULL, cmd->resp, 0);
			cmd->resp[0] = swab32(cmd->resp[0]);
			debug("r32 %x\n", cmd->resp[0]);
			break;
		case MMC_CMD_SEND_STATUS:
			spi_xfer(spi, 1 * 8, NULL, cmd->resp, 0);
			cmd->resp[0] = (cmd->resp[0] & 0xff) ?
