 ...
 */
static int
mxsmmc_request(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data)
{
	...
	if (cmd->flags & MMC_RSP_PRESENT)	/* Need to get response */
		ctrl0 |= SSP_CTRL0_GET_RESP;
	if (cmd->flags & MMC_RSP_136)	/* It's a 136 bits response */
	...
	if (cmd->flags & MMC_RSP_136) {
		cmd->resp[3] = readl(&ssp_regs->hw_ssp_sdresp0);
		cmd->resp[2] = readl(&ssp_regs->hw_ssp_sdresp1);
		cmd->resp[1] = readl(&ssp_regs->hw_ssp_sdresp2);
		cmd->resp[0] = readl(&ssp_regs->hw_ssp_sdresp3);
	} else
		cmd->resp[0] = readl(&ssp_regs->hw_ssp_sdresp0);
