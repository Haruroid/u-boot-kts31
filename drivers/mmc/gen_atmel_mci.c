 ...
 */
static int
mci_request(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data)
{
	atmel_mci_t *mci = (atmel_mci_t *)mmc->priv;
	u32 cmdr;
	u32 error_flags = 0;
	...
	if (cmd->flags & MMC_RSP_136) {
		cmd->resp[0] = readl(&mci->rspr);
		cmd->resp[1] = readl(&mci->rspr1);
		cmd->resp[2] = readl(&mci->rspr2);
		cmd->resp[3] = readl(&mci->rspr3);
	} else
		cmd->resp[0] = readl(&mci->rspr);
