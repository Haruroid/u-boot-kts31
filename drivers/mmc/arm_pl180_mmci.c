...
static int wait_for_command_end(struct mmc *dev, struct mmc_cmd *cmd)
{
	...
	if (cmd->flags & MMC_RSP_PRESENT) {
		cmd->resp[0] = readl(&host->base->response0);
		cmd->resp[1] = readl(&host->base->response1);
		cmd->resp[2] = readl(&host->base->response2);
		cmd->resp[3] = readl(&host->base->response3);
		debug("CMD%d response[0]:0x%08X, response[1]:0x%08X, "
			"response[2]:0x%08X, response[3]:0x%08X\n",
			cmd->cmdidx, cmd->resp[0], cmd->resp[1],
