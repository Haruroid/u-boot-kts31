...
sdh_send_cmd(struct mmc *mmc, struct mmc_cmd *mmc_cmd)
{
	...
	if (flags & MMC_RSP_PRESENT) {
		mmc_cmd->resp[0] = bfin_read_SDH_RESPONSE0();
		if (flags & MMC_RSP_136) {
			mmc_cmd->resp[1] = bfin_read_SDH_RESPONSE1();
			mmc_cmd->resp[2] = bfin_read_SDH_RESPONSE2();
			mmc_cmd->resp[3] = bfin_read_SDH_RESPONSE3();
		}
	}
