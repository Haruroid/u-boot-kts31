...
static inline void MY_CLR_ALIGN_BUFFER(){
	alloc_ptr = 0x200000;
}

#define MY_ALLOC_CACHE_ALIGN_BUFFER(type, name, size)	type* name = alloc_outside_buffer(ROUND(size * sizeof(type), ARCH_DMA_MINALIGN));
...
/* RTK 2nd stage bootloader support */
       ...
       extern unsigned int golden_sec_start;
       extern unsigned int golden_sec_end;
  #else
